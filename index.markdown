...
It is worth pointing out that the the address of the `ReplaceSel()` function in Windbg ends in `eb38` and not `57a8` as you might expect, since we just said its offset is `0x157a8`. This is only because IDA is showing the version of notepad.exe from the target system, while Windbg is running my local version of notepad.exe, and there's no guarantee functions will be at the same address across different versions of the same application.
