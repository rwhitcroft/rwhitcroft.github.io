...
# More Failure
Because the shellcode file is stored on disk, all I had to do was replace the MessageBox shellcode file with a reverse shell shellcode file. I opted to use a [revshell.py](custom reverse shell payload) instead of one from `msfvenom` for fear that it would get caught on disk. I won't go into the custom reverse shell in this document but suffice it to say it is similar in every way except for the arrangement of bytes that would cause a pattern match.
