...
To be able to write to memory, the first task was to locate the base address of `InProcessClient64.dll`. Due to ASLR, this address will be different on every system, and after every reboot. I created a C++ console app in Visual Studio whose sole purpose (for now) is to attack the `InProcessClient64.dll` that gets injected into it when the process is created.
