...
## Function Hunting
Now that I know what I want to write (`ret`), all that was left was finding the function to "patch". The total number of functions in `InProcessClient64.dll` is 7,447, so I had my work cut out for me. I tried a few different ways to automate this task but ended up doing it manually.

![idafunc](/images/idafunc.png)

I used IDA to export the list of all 7,447 functions to a text file, isolated the start address of each function, then split the list into chunks of 100. Instead of programmatically writing a `ret` (since I just proved that I could), I could cheat and use WinDbg's `eb` command (edit byte) to quickly patch a `ret` instruction at each of the 100 functions for this iteration. I also switched my test application to one that tries to run the `windows/x64/messagebox` payload since it would certainly get detected as malicious, but should run without detection if I happened to blind the right function.
